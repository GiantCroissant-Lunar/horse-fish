...
- **Agent Name:** pi-sqlite-persist
- **Task ID:** horse-fish-9afc
- **Spec:** No spec file provided
- **Branch:** overstory/pi-sqlite-persist/horse-fish-9afc
- **Worktree:** /Users/apprenticegc/Work/lunar-horse/yokan-projects/horse-fish/.overstory/worktrees/pi-sqlite-persist
- **Parent:** coordinator
- **Depth:** 0
...
Your worktree root is: `/Users/apprenticegc/Work/lunar-horse/yokan-projects/horse-fish/.overstory/worktrees/pi-sqlite-persist`

**CRITICAL**: All file operations MUST use paths within this directory.
- Use paths relative to your worktree root, or absolute paths starting with `/Users/apprenticegc/Work/lunar-horse/yokan-projects/horse-fish/.overstory/worktrees/pi-sqlite-persist`
...
These paths are relative to your worktree root: `/Users/apprenticegc/Work/lunar-horse/yokan-projects/horse-fish/.overstory/worktrees/pi-sqlite-persist`
...
Use `ov mail` for all communication. Your address is **pi-sqlite-persist**.

```bash
# Check your inbox (do this regularly)
ov mail check --agent pi-sqlite-persist

# Send a status update to your parent
ov mail send --to coordinator --subject "status" \
  --body "Progress update here" --type status --agent pi-sqlite-persist

# Ask a question
ov mail send --to coordinator --subject "question" \
  --body "Your question here" --type question --priority high --agent pi-sqlite-persist

# Report completion
ov mail send --to coordinator --subject "done" \
  --body "Summary of what was done" --type result --agent pi-sqlite-persist

# Reply to a message
ov mail reply <message-id> --body "Your reply" --agent pi-sqlite-persist
```
...
4. **Commit:** all changes committed to your branch (overstory/pi-sqlite-persist/horse-fish-9afc)
5. **Record mulch learnings:** `ml record <domain> --type <convention|pattern|failure|decision> --description "..." --outcome-status success --outcome-agent pi-sqlite-persist` — capture insights from your work
6. **Signal completion:** send `worker_done` mail to coordinator: `ov mail send --to coordinator --subject "Worker done: horse-fish-9afc" --body "Quality gates passed." --type worker_done --agent pi-sqlite-persist`
7. **Close issue:** `sd close horse-fish-9afc --reason "summary of changes"`
...
- **WORKTREE ISOLATION**: All writes MUST target files within your worktree at `/Users/apprenticegc/Work/lunar-horse/yokan-projects/horse-fish/.overstory/worktrees/pi-sqlite-persist`
- NEVER write to the canonical repo root — all writes go to your worktree copy
- Only modify files in your File Scope
- Commit only to your branch: overstory/pi-sqlite-persist/horse-fish-9afc
